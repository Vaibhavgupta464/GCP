...
const ARBITRUM_SEPOLIA_MULTICALL_ADDRESS = '0x2B718b475e385eD29F56775a66aAB1F5cC6B2A0A'

export function useInterfaceMulticall(chainId?: UniverseChainId) {
  const account = useAccount()
  const chain = chainId ?? account.chainId
  const multicallAddress =
    chain === UniverseChainId.ArbitrumSepolia
      ? ARBITRUM_SEPOLIA_MULTICALL_ADDRESS
      : chain
